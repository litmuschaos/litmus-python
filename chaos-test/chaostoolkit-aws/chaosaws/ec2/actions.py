...
__all__ = ["stop_instance", "stop_instances", "terminate_instances",
           ...
           "detach_random_volume", "attach_volume", "execute_ssm_command"]
...
def execute_ssm_command(instance_id: str,
                    commands: List[str],
    ...
    logger.info("Command sent , id ==> " + command_id)

    wait_max_retries = 3
    status = None

    logger.info("Explicity waiting for 3 seconds post SSM command")
    time.sleep(3)
    while True:
        ...
        try:
            cp = response_list['CommandInvocations'][0]['CommandPlugins'][0]
            status = cp['StatusDetails']

            if status == "Failed":
                logger.info("Command failed, response is " + cp['Output'])
                break
            elif status == "Success":
                logger.info("Command succeeded, response is " + cp['Output'])
                break
            elif status == "InProgress":
                logger.info("Execution state is in progress, will wait for " + wait_time_duration_in_secs
                            + " seconds and retry for " + str(wait_max_retries) + " time")
                time.sleep(wait_time)
                wait_max_retries -= 1
            elif status == "Pending":
                logger.info("Execution state is in Pending, will wait for " + wait_time_duration_in_secs
                            + " seconds and retry for " + str(wait_max_retries) + " time")
                time.sleep(wait_time)
                wait_max_retries -= 1
            elif status == "Delayed":
                logger.info("Execution state is Delayed, will wait for " + wait_time_duration_in_secs
                            + " seconds and retry for " + str(wait_max_retries) + " time")
                time.sleep(wait_time)
                wait_max_retries -= 1
        except Exception:
            logger.info("Exception in getting value, will wait for " + wait_time_duration_in_secs
                        + " seconds and retry for " + str(wait_max_retries) + " time")
            time.sleep(wait_time)
            wait_max_retries -= 1
