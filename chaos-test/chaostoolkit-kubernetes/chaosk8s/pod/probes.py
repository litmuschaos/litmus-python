...
def pods_in_phase(label_selector: str, phase: str = "Running",
                  ns: str = "default", secrets: Secrets = None, expected_pod_count: int = 0) -> bool:
    """
    ...
    if expected_pod_count is not 0:
        matched_pods = 0
        for pod in ret.items:
            if pod.status.phase == phase:
                matched_pods += 1
        if matched_pods < expected_pod_count:
            raise ActivityFailed("Number of pods matching expected status {s} is wrong, "
                                 "expected {d} found {n}".format(s=phase, d=expected_pod_count, n=matched_pods))
        return True
...
def count_pods_string(label_selector: str, phase: str = None,
               ns: str = "default", secrets: Secrets = None) -> str:
    """
    Count the number of pods matching the given selector in a given `phase`, if
    one is given.
    """
