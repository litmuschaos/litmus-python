...
def PreparePodDelete(experimentsDetails , resultDetails, eventsDetails, chaosDetails):
	...
	if experimentsDetails.RampTime != 0 :
		print("[Ramp]: Waiting for the %vs ramp time before injecting chaos", experimentsDetails.RampTime)
	...
	if experimentsDetails.RampTime != 0 :
		print("[Ramp]: Waiting for the %vs ramp time after injecting chaos", experimentsDetails.RampTime)
		common.WaitForDuration(experimentsDetails.RampTime)
	return None
...
def injectChaosInSerialMode(experimentsDetails , clients , chaosDetails , eventsDetails , resultDetails): 
	...
	while duration < experimentsDetails.ChaosDuration:
		...
		targetPodList, err, count = pods.GetPodList(experimentsDetails.TargetPods, experimentsDetails.PodsAffectedPerc, chaosDetails)
		if err != None: 
			return err
		print("target Pods : ", count)
		podNames = []
		if count == 1:
		...
		print("Target pods list, %v", podNames)
	...
	print("[Completion]: %v chaos is done", experimentsDetails.ExperimentName)
...
def injectChaosInParallelMode(experimentsDetails  , chaosDetails , eventsDetails , resultDetails):
	...
	while duration < experimentsDetails.ChaosDuration:
		...
		print("Target pods list for chaos, %v", podNames)
		print("Names Pods", podNames)
		if experimentsDetails.EngineName != "" :
			msg = "Injecting " + experimentsDetails.ExperimentName + " chaos on application pod"
			types.SetEngineEventAttributes(eventsDetails, types.ChaosInject, msg, "Normal", chaosDetails)
