...
def injectChaosInSerialMode(experimentsDetails , clients , chaosDetails , eventsDetails , resultDetails): 
	...
	while duration < experimentsDetails.ChaosDuration:
		...
		for pod in targetPodList.items :
			...
			logger.info("[Info]: Killing the following pods", "PodName :", pod.name)
			if experimentsDetails.Force == True:
				err = v1.delete_namespaced_pod(pod.name, experimentsDetails.AppNS, grace_period_seconds=GracePeriod)
			else:
				err = v1.delete_namespaced_pod(pod.name, experimentsDetails.AppNS)
